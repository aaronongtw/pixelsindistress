...
window.OurGame.dayReport = function(gameState, startNextDayFn, buyNurse, buyChocolates) {
  ...
  for (var i = 0; i <gameState.todaysPeople.length; i++) {
    ...
    choices.push(<li key={i}>{gameState.todaysPeople[i].name},<br/> Change in Stress:
      <span className={stressChange <= 0 ? "good":"bad"}>{stressChange}</span><br/> Status: <span className={currentStress <gameState.todaysPeople[i].maxStress ? "good":"bad"}>{(currentStress < 1) ? "HAPPY!" : currentStress <gameState.todaysPeople[i].maxStress ? "Functioning" : 'Intensive Care'}</span> </li>);
